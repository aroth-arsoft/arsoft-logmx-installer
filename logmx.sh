...
# Get LogMX directory
LOGMX_PATH='/usr/lib/logmx'
[ ! -d "$HOME/.config/logmx" ] && mkdir "$HOME/.config/logmx"

# Set default startup config
STARTUP_CONF_FILE="$LOGMX_PATH/startup.conf"
SPLASH_SCREEN="$LOGMX_PATH/pics/splash_screen.png"
MAX_MEMORY=400m
CONFIG_FILE="-Dconfig.file=$HOME/.config/logmx/logmx.properties"
LOGGING_PARAM=""
...
# Start LogMX
exec -a "$0" java -Xmx$MAX_MEMORY -splash:"$SPLASH_SCREEN" $LOGGING_PARAM "$CONFIG_FILE" -cp "$LOGMX_CLASSPATH" $LOGMX_MAIN "$@"
